...
"""
#generator for tests only. does not work
def Generator():
  inputs = tf.keras.layers.Input(shape=[256, 256, 3])

  down_stack = [
    downsample(64, 4, apply_batchnorm=False),  # (batch_size, 128, 128, 64)
    downsample(256, 16),  # (batch_size, 32, 32, 256)
    downsample(512, 16),  # (batch_size, 8, 8, 512)
    downsample(512, 16),  # (batch_size, 2, 2, 512)
    downsample(512, 4),  # (batch_size, 1, 1, 512)
  ]

  up_stack = [
    upsample(512,  4, apply_dropout=True),  # (batch_size, 2, 2, 1024)
    upsample(512, 16, apply_dropout=True),  # (batch_size, 8, 8, 1024)
    upsample(256, 16),  # (batch_size, 32, 32, 512)
    upsample(64,  16),  # (batch_size, 128, 128, 128)
  ]
...
def load(image_file):
      ...
      R = 0.299*real_image[:,:,0] 
      G = 0.587*real_image[:,:,1] 
      B = 0.114*real_image[:,:,2]
  
      gray = R+G+B

      input_image = tf.stack((gray,gray,gray),-1)
      input_image = tf.cast(input_image, tf.uint8)
      #print("input image: ", input_image.shape )
      input_image = tf.cast(input_image, tf.float32)

      #print("real image after cast: ", real_image.shape)
      #print("input_image after cast: ",input_image.shape)
      return input_image, real_image
...
PATH = "C:\\python\\pix2pix\\coloring\\"
...
my_dataset = tf.data.Dataset.list_files(PATH + 'myFaces\\NEW*.*')
my_dataset = my_dataset.map(load_image_test)
my_dataset = my_dataset.batch(BATCH_SIZE)

for inp, tar  in my_dataset.take(7): 
    print (inp.shape)
    print (tar.shape)
